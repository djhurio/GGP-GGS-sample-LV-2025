...
Ietvarā ir `r frame.csp[, prettyNum(.N, big.mark = ",")]` mājokļi, no kuriem `r frame.csp[, prettyNum(sum(vc1859_sum == 0L), big.mark = ",")]` (`r oc.rate` %) mājokļos nav reģistrēta neviena persona vecumā 18--59 gadi.
...
Kopā VZD datos pēc datu apstrādes ir `r tab_majo_vzd[, prettyNum(sum(N), big.mark = ",")]` adresācijas objekti, kas ietver ēkas un telpu grupas (dzīvokļus). Ņemt vērā, ka šie dati satur visus adresācijas objektus - tai skaitā industriālās ēkas un neapdzīvotus mājokļus.

```{r VZD table}
gt(tab_majo_vzd[, .(tips_cd, nosaukums, N)]) |>
  tab_header("Adresācijas objekti VZD Adrešu reģistra datos") |>
  cols_label(
    tips_cd = "Kods",
    nosaukums = "Adresācijas objekta tips",
    N = "Skaits"
  ) |>
  grand_summary_rows(
    columns = "N",
