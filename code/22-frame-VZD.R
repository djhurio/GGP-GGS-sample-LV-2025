...
frame_dziv <- aw_dziv[
  statuss == "EKS",
  .(kods, tips_cd, vkur_cd, vkur_tips, nosaukums, sort_nos)
]
...
tab2 <- melt.data.table(
  data = tab0,
  id.vars = "kods",
  measure.vars = vnames2,
  variable.name = "id",
  variable.factor = T,
  value.name = "nosaukums",
  na.rm = T
)
...
tab3 <- melt.data.table(
  data = tab0,
  id.vars = "kods",
  measure.vars = vnames3,
  variable.name = "id",
  variable.factor = T,
  value.name = "atrib",
  na.rm = T
...
tab5 <- dcast.data.table(
  data = tab4,
  formula = kods ~ tips_cd,
...
frame_ter_2 <- merge(
  x = frame_ter[, .(kods, tips_cd_ter = tips_cd)],
  y = tab5,
  by = "kods"
)
frame_ter_2
...
frame_eka[
  !grepl("^[0-9]+[A-Z]?( k-[0-9]+[A-Z]?)?$", nosaukums),
...
setnames(
  frame_eka,
  c("nosaukums", "sort_nos", "atrib", "vkur_cd"),
...
frame_majo <- rbindlist(
  list(frame_eka, frame_dziv2),
  fill = T
)
...
# save(
#   frame_majo_vzd,
#   file = file.path(config::get("dir.data"), "frame_majo_vzd.Rdata")
# )

fwrite(
  x = frame_majo_vzd,
  file = file.path(config::get("dir.data"), "frame_majo_vzd.csvy.gz"),
  yaml = TRUE
)
